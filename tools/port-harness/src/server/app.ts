...
import { createJobsRoutes, getJobQueue } from "./routes/jobs.js";
...
export function createApp(db: Database.Database, config: HarnessConfig): Hono {
  ...
  const queue = getJobQueue(db, config);

  app.route("/api/tasks", createTasksRoutes(db));
  app.route("/api/flows", createFlowsRoutes(db, config, queue));
  app.route("/api/symbols", createSymbolsRoutes(db, config));
  app.route("/api/jobs", createJobsRoutes(db, config, queue));
  app.route("/api/stats", createStatsRoutes(db));
  app.route("/api/files", createFilesRoutes(db, config, queue));
  app.route("/api/discover", createDiscoverRoutes(db, config, queue));
