...
use crate::game::auction::{
    ...
    send_auction_removed_notification, PlaceBidResult,
};
...
pub async fn handle_auction_place_bid(
    ...
    match result {
        PlaceBidResult::Success(outcome) => {
            send_auction_command_result(
                session,
                Some(&outcome.auction),
                AuctionAction::BidPlaced,
                AuctionError::Ok,
                None,
            )?;
        }
        PlaceBidResult::Rejected { error, auction } => {
            send_auction_command_result(
                session,
                auction.as_ref(),
                AuctionAction::BidPlaced,
    ...
    fn make_player_mgr(with_feign_death: bool, auction_access_mode: i8) -> Arc<PlayerManager> {
        ...
        if with_feign_death {
            let aura = Aura::new(
                ...
                player_guid,
                0,
                crate::game::player::auras::effects::AURA_DUMMY,
                0,
                0,
                Some(60_000),
                0,
