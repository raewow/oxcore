...
use crate::game::chat::ChatSystem;
use crate::game::combat::CombatSystem;
use crate::game::coordination::{LinkingManager, LinkingSystem, PoolManager, PoolSystem};
use crate::game::creature::ai::AIEventQueue;
use crate::game::creature::movement::MovementSystem;
use crate::game::creature::respawn::RespawnSystem;
use crate::game::creature::{AddonManager, AddonSystem, CreatureManager};
use crate::game::grid_system::GridSystem;
use crate::game::group::GroupSystem;
use crate::game::guild::GuildSystem;
use crate::game::inventory::InventorySystem;
use crate::game::loot::{LootManager, LootSystem};
use crate::game::npc::{
...
use crate::game::player::power::PowerSystem;
use crate::game::player::reputation::ReputationSystem;
use crate::game::player::settings::SettingsSystem;
use crate::game::player::skills::SkillSystem;
use crate::game::player::spells::SpellSystem;
use crate::game::player::stats::StatsSystem;
use crate::game::player::talents::TalentSystem;
...
impl SystemManager {
    pub fn new(
        ...
        let talents = Arc::new(TalentSystem::new(std::sync::Arc::new(
            crate::game::player::talents::TalentStore::load(vec![], vec![]),
        )));
        ...
        let honor = Arc::new(crate::game::player::honor::HonorSystem::new());
    ...
    pub fn init_talents_from_dbc(&self, dbc_mgr: &crate::dbc::DbcManager) {
        use crate::game::player::talents::TalentStore;

        // Collect talent entries from DBC
        let talent_entries: Vec<(u32, crate::dbc::structures::TalentEntry)> = dbc_mgr
            .get_all_talents()
            .map(|(id, entry)| (*id, entry.clone()))
            .collect();

        // Collect talent tab entries from DBC
        let talent_tab_entries: Vec<(u32, crate::dbc::structures::TalentTabEntry)> = dbc_mgr
            .get_all_talent_tabs()
            .map(|(id, entry)| (*id, entry.clone()))
            .collect();
