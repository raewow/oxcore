...
/// misc_value = lock ID
pub async fn effect_open_lock(input: &EffectInput, _world: &World) -> Result<EffectResult> {
    ...
    tracing::debug!(
        "Open lock: caster={:?} target={:?} lock={}; loot follows SPELL_GO",
        input.caster_guid,
        target_guid,
        lock_id
    );
...
pub async fn effect_summon_object_slot1(
    input: &EffectInput,
    world: &World,
) -> Result<EffectResult> {
...
pub async fn effect_summon_object_slot2(
    input: &EffectInput,
    world: &World,
) -> Result<EffectResult> {
...
pub async fn effect_summon_object_slot3(
    input: &EffectInput,
    world: &World,
) -> Result<EffectResult> {
...
pub async fn effect_summon_object_slot4(
    input: &EffectInput,
    world: &World,
) -> Result<EffectResult> {
