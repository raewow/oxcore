...
pub async fn process_deaths(world: &World) -> anyhow::Result<()> {
    ...
    if !just_died.is_empty() {
        tracing::info!(
            "[DEATH] process_deaths: found {} creatures in JustDied state",
            just_died.len()
        );
    }
...
async fn process_creature_death(world: &World, guid: ObjectGuid) -> anyhow::Result<()> {
    ...
    let Some(Some((position, entry, loot_recipient, map_id, instance_id))) = info else {
        if info.is_none() {
            tracing::warn!(
                "[DEATH] process_creature_death: creature {:?} not found!",
                guid
            );
        } else {
            tracing::info!(
                "[DEATH] process_creature_death: {:?} already has loot, skipping",
                guid
            );
        }
        return Ok(());
    ...
    if let Some(recipient_guid) = loot_recipient {
        tracing::info!(
            "[DEATH] process_creature_death {:?}: has loot recipient {:?}, generating loot",
            guid,
            recipient_guid
        );
        ...
        world
            .systems
            .loot
            .generate_creature_loot_on_death(guid, world)
            .await?;
        ...
        tracing::info!(
            "[DEATH] process_creature_death {:?}: no loot recipient, clearing dynflags",
            guid
        );
