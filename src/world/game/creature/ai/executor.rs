...
fn execute_single_action(
    ...
    match action {
        ...
        AIAction::MoveToTarget { target_guid, min_distance: _ } => {
            ...
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                ...
                creature.motion_master.chase(target_guid, creature_guid, current_pos, combat_reach, run_speed);
            });
        ...
        AIAction::MoveTo { position, movement_type } => {
        ...
        AIAction::ReturnToSpawn => {
            ...
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                ...
                let run_speed = creature.run_speed();
                creature.motion_master.return_home(home, creature_guid, current_pos, run_speed);
        ...
        AIAction::LeaveCombat => {
            ...
            let restore_wander = world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                ...
                (creature.wander_distance, creature.home_position, creature.position)
            });
                ...
                if wander_dist > 0.0 {
                    world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                        ...
                        creature.motion_master.random_wander(
                            home_pos, wander_dist, creature_guid, current_pos, walk_speed,
                        );
                    });
                }
            }
        ...
        AIAction::EnterCombat { target_guid } => {
            ...
            let was_already_in_combat = world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
            ...
            world.systems.creature_movement.send_stop_packet(
                creature_guid, current_pos, world,
            );
            ...
            if !was_already_in_combat {
                super::system::queue_event(
                    world,
                    creature_guid,
                    super::types::AIEvent::CombatStarted { initial_aggressor: target_guid },
                );
            }
        ...
        AIAction::FleeFrom { flee_from_guid, distance, duration_ms } => {
            // Movement comes in Phase 8
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
        ...
        AIAction::FaceTarget { target_guid } => {
            ...
            let creature_pos = world.managers.creature_mgr
                .get_position(creature_guid)
                .unwrap_or_default();
            ...
            if let Some(target_pos) = target_pos {
                let angle = (target_pos.y - creature_pos.y)
                    .atan2(target_pos.x - creature_pos.x);
        ...
        AIAction::CastSpell { spell_id, target_guid, target_position: _, triggered } => {
            execute_creature_spell_cast(world, creature_guid, spell_id, target_guid, triggered);
        }

        AIAction::AddThreat { target_guid, amount } => {
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
        ...
        AIAction::ModifyThreat { target_guid, amount, is_percent } => {
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                ...
                if let Some(entry) = creature.combat.threat_list.iter_mut().find(|e| e.guid == target_guid) {
                    if is_percent {
                        entry.threat *= amount;
                    } else {
                        entry.threat = amount;
        ...
        AIAction::CallForHelp { radius } => {
            tracing::debug!("[AI] Creature {:?} calling for help ({} yards)", creature_guid, radius);
        ...
        AIAction::PlaySound { sound_id, zone_wide: _ } => {
        ...
        AIAction::SetFaction { faction_id } => {
            ...
            tracing::debug!("[AI] Creature {:?} faction set to {}", creature_guid, faction_id);
        }
        ...
        AIAction::SetImmune { physical, spell } => {
            ...
            tracing::debug!("[AI] Creature {:?} immune set: phys={}, spell={}", creature_guid, physical, spell);
        }
        ...
        AIAction::SetHealthPercent { percent } => {
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                creature.current_health = ((creature.max_health as f32) * percent.clamp(0.0, 1.0)) as u32;
            });
            send_health_update(world, creature_guid);
            tracing::debug!("[AI] Creature {:?} health set to {}%", creature_guid, (percent * 100.0) as u32);
        }
        ...
        AIAction::Morph { display_id } => {
            ...
            tracing::debug!("[AI] Creature {:?} morphed to display {}", creature_guid, display_id);
        }

        AIAction::Demorph => {
            let native_display = world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
        ...
        AIAction::SpawnCreature { entry, position, summon_type: _, duration_ms: _ } => {
            // Spawn creature at position - uses creature manager
            tracing::debug!(
                "[AI] Creature {:?} spawning creature entry {} at ({}, {}, {})",
                creature_guid, entry, position.x, position.y, position.z
            );
        ...
        AIAction::SpawnGameObject { entry, position, duration_secs } => {
            tracing::debug!(
                "[AI] Spawning gameobject entry {} at ({}, {}, {}) for {}s",
                entry, position.x, position.y, position.z, duration_secs
            );
            // TODO: Full gameobject spawn implementation
        }

        AIAction::RespawnGameObject { guid, duration_secs } => {
            tracing::debug!("[AI] Respawning gameobject {:?} for {}s", guid, duration_secs);
        ...
        AIAction::SetInstanceData { data_id, value } => {
            ...
            if let Some((map_id, instance_id)) = world.managers.creature_mgr.with_creature(creature_guid, |c| {
                (c.map_id, c.instance_id)
            }) {
                world.managers.lua_mgr.set_instance_data(map_id, instance_id, data_id, value);
                tracing::debug!("[AI] Set instance data: {} = {} (map={}, inst={})", data_id, value, map_id, instance_id);
            }
        }
        ...
        AIAction::SetInstanceGuid { data_id, guid } => {
            if let Some((map_id, instance_id)) = world.managers.creature_mgr.with_creature(creature_guid, |c| {
                ...
                let mut state = world.managers.lua_mgr.get_instance_state(map_id, instance_id);
                state.set_guid(data_id, guid);
                world.managers.lua_mgr.set_instance_state(map_id, instance_id, state);
                tracing::debug!("[AI] Set instance GUID: {} = {:?} (map={}, inst={})", data_id, guid, map_id, instance_id);
            }
        }
        ...
        AIAction::RemoveUnitFlag { flag } => {
            ...
            tracing::debug!("[AI] Remove unit flag 0x{:X} from {:?}", flag, creature_guid);
        }
        ...
        AIAction::SetCombatWithZone => {
            ...
            if let Some(map_id) = world.managers.creature_mgr.with_creature(creature_guid, |c| c.map_id) {
                ...
                tracing::debug!("[AI] Set combat with zone: {} players engaged on map {}", player_guids.len(), map_id);
            }
        }
        ...
        AIAction::MoveAlongPath { waypoints, movement_type, repeating } => {
            use crate::world::game::creature::movement::generators::Waypoint;

            let wps: Vec<Waypoint> = waypoints.iter().enumerate().map(|(i, pos)| {
                Waypoint {
                    point_id: i as u32,
                    position: *pos,
                    wait_time: 0,
            ...
            tracing::debug!("[AI] Move along path ({} waypoints, repeating={})", waypoints.len(), repeating);
        }
...
fn send_health_update(world: &World, creature_guid: ObjectGuid) {
    ...
    if let Some((current_health, max_health)) = world.managers.creature_mgr
...
fn send_combat_state_update(world: &World, creature_guid: ObjectGuid, target_guid: ObjectGuid) {
    ...
    if let Some(raw_flags) = world.managers.creature_mgr
...
pub fn execute_creature_spell_cast(
    ...
    let spell_entry = match world.managers.spell_mgr.get(spell_id) {
        Some(s) => s,
        None => {
            tracing::warn!("[AI] Creature {:?} tried to cast unknown spell {}", creature_guid, spell_id);
            return;
        }
    };
    ...
    if spell_entry.mana_cost > 0 {
        let has_mana = world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
        ...
        if !has_mana {
            tracing::debug!("[AI] Creature {:?} not enough mana for spell {}", creature_guid, spell_id);
            return;
        }
    }
        ...
        match effect_type {
            2 => {
                ...
                if let Some(target) = target_guid {
                    ...
                    apply_creature_spell_damage(world, creature_guid, target, damage, spell_id, school);
                }
            }
            10 => {
                // SPELL_EFFECT_HEAL
            ...
            _ => {
                ...
                if effect_type != 0 {
                    tracing::debug!(
                        ...
                        creature_guid, spell_id, effect_idx, effect_type
                    );
                }
            }
        }
    ...
    if !triggered {
        let cooldown_ms = spell_entry.recovery_time.max(spell_entry.category_recovery_time);
        if cooldown_ms > 0 {
            world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
                creature.ai_state_data.set_spell_cooldown(spell_id, cooldown_ms);
            });
        }
    }
    ...
    tracing::debug!(
        ...
        creature_guid, spell_id, target_guid, triggered
    );
}
...
fn apply_creature_spell_damage(
    ...
    if target_guid.is_player() {
        ...
        world.managers.creature_mgr.apply_damage(target_guid, damage, caster_guid, timestamp);
    }
}
...
fn apply_creature_spell_heal(
    ...
    if target_guid.is_creature() {
        world.managers.creature_mgr.with_creature_mut(target_guid, |creature| {
            creature.current_health = (creature.current_health + heal_amount).min(creature.max_health);
        });
...
fn send_creature_spell_damage_log(
    ...
    use crate::shared::protocol::{WorldPacket, Opcode};
    ...
    if target_guid.is_player() {
        world.managers.broadcast_mgr.broadcast_nearby(target_guid, &packet, true);
    } else {
...
fn send_creature_chat(world: &World, creature_guid: ObjectGuid, chat_type: u8, text: &str) {
    ...
    let name = world.managers.creature_mgr
