...
use super::aggro::{calculate_aggro_range, should_aggro_creature, is_npc, is_valid_aggro_target, distance_squared_2d, MAX_AGGRO_RANGE};
use super::types::{AIState, AIEvent};
...
pub async fn scan_for_aggro(world: &World) -> anyhow::Result<()> {
    ...
    let idle_creatures: Vec<(ObjectGuid, u32, u32, Position, u8, u32, u32, u32, AIState, bool)> = world.managers.creature_mgr
        .iter_creatures()
        .filter(|entry| {
    ...
    for (creature_guid, map_id, instance_id, pos, level, faction, unit_flags, npc_flags, _, _) in idle_creatures {
...
async fn check_creature_aggro(
    ...
    for target_guid in nearby {
        ...
        // Get player info
        let player_info = world.managers.player_mgr.with_player(target_guid, |player| {
            ...
            let player_faction = if player.race == 1 || player.race == 3 || player.race == 4 || player.race == 7 || player.race == 11 {
                1 // Alliance
            } else {
                2 // Horde
            };
        ...
        let is_hostile = should_aggro_creature(creature_faction, creature_flags, player_faction, true);
        ...
        if dist_sq <= aggro_range * aggro_range {
            ...
            if !world.managers.vmap_mgr.is_in_line_of_sight(map_id, creature_pos, player_pos) {
                continue;
            }
            ...
            if world.managers.lua_mgr.has_creature_ai(
                world.managers.creature_mgr.with_creature(creature_guid, |c| c.entry).unwrap_or(0)
            ) {
                queue_event(
                    world,
                    creature_guid,
