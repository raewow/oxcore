...
use crate::world::game::creature::combat::{
    roll_melee_hit_outcome, calculate_melee_damage, apply_hit_outcome,
    hit_outcome_to_hit_info, hit_outcome_to_victim_state, MeleeHitOutcome,
};
...
pub fn perform_creature_melee_attack(
    ...
    let creature_data = world.managers.creature_mgr.with_creature_mut(creature_guid, |creature| {
    ...
    let target_data = world.managers.player_mgr.with_player_mut(target_guid, |player| {
        if !player.is_alive() {
            return None;
        }
        Some((
    ...
    let (target_level, target_armor, can_parry, can_block, target_pos) =
        match target_data {
            Some(data) => data,
            None => return,
        };
    ...
    let hit_outcome = roll_melee_hit_outcome(
        attacker_level,
        target_level,
        can_parry,
        can_block,
    ...
    if damage > 0 {
        let new_health = world.managers.player_mgr.with_player_mut(target_guid, |player| {
    ...
    if target_died {
        ...
        if let Err(e) = world.systems.death.on_killed(target_guid, Some(creature_guid), None, world) {
...
fn send_player_health_update(world: &World, player_guid: ObjectGuid) {
    let health_data = world.managers.player_mgr.with_player_mut(player_guid, |player| {
...
fn handle_creature_kill_cleanup(
    world: &World,
    player_guid: ObjectGuid,
    killer_guid: ObjectGuid,
) {
    // Remove player from all creature threat lists
    let creature_guids: Vec<ObjectGuid> = world.managers.creature_mgr
        .iter_creatures()
        .filter(|creature| {
    ...
    for cg in creature_guids {
        world.managers.creature_mgr.with_creature_mut(cg, |creature| {
